#include <stdio.h>
#include <malloc.h>
#define MazeScale 10 //迷宫最大行列数均为10
...
int main()
{
    ...
    printf("请输入迷宫的列数：");
	scanf("%d",&n);
	printf("请输入迷宫（墙元素为1，可走路径为0,迷宫四周均为1）\n");	 
	for(i=0;i<m;i++)
    {
        for(j=0;j<n;j++)
        {
    ...
    do
    {
        if(Pass(curp))
        {
            FootPrint(curp,3);//可走就在迷宫里面留下足迹,路径标记为3 
            ...
            //找到下一个被试块 
            curp=NextPosition(curp,1);//找到前一个被试块下面的路径块作为被试块
            curStep++;//被探索的步数加一
